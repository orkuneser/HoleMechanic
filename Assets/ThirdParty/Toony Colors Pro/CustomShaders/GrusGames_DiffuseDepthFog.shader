Shader "GrusGames/Fog/Depth/Fog"
{
	Properties
	{
		...
		_Tiling ("Fog tiling", Vector) = (1, 1, 1, 1)
		...
		[Space(20)]
		//_DeepColor ("Fog deep color", Color) = (1, 1, 1, 1)
		_fogHeight ("Fog height", Float) = 0
		_fogDeep ("Fog deep", Float) = 0
		_fogDepth ("Fog depth param", Range(0, 0.1)) = 0
		_fogMinAlpha ("Fog min alpha", Range(0, 1)) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata v)
			{
				...
				o.camHeightOverWater = _WorldSpaceCameraPos.y -_fogHeight;
				...
				o.waterDepth = depth * _fogDepth * saturate(rayToWaterPlane.y - camToWorldRay.y);;
				...
				fixed3 positionForFog = lerp(worldPosOnPlane, o.worldPos.xyz, o.worldPos.y >_fogHeight);
			...
			{
				fixed lengthUnderWater = max(0, _fogHeight - i.worldPos.y);
				...
				fixed waterAlpha = saturate(lengthUnderWater / _fogDeep + _fogMinAlpha + i.waterDepth);
				...
				fixed4 waterCol = tex2D(_fogTex, distorted_uv);
				waterCol = lerp(_fogColor, fixed4(1, 1, 1, 1), waterCol.r * _TextureVisibility);

				fixed4 finalCol = lerp(mainCol, waterCol, _fogColor.a * waterAlpha * underWater);
